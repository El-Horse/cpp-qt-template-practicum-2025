...
void MainWindow::on_pb_find_clicked(){
    std::string needle = ui->txt_elem_content->text().toStdString();
    vector_model_.iterator = std::find(vector_model_.items.begin(), vector_model_.items.end(),needle);
    ApplyIterator();
}


void MainWindow::on_pb_count_clicked(){
    std::string needle = ui->le_count->text().toStdString();
    int count = std::count_if(vector_model_.items.begin(),vector_model_.items.end(),[&](std::string& str){
        return str == needle;
    });
    ui->lbl_count->setText(QString::fromStdString("%1").arg(count));
}
